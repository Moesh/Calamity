...
execute as @a[tag=Playing,scores={prepScore=1..}] run tellraw @a {"translate":"%s %s +%s points","color":"light_gray","with":[{"text":">>>","color":"gray"},{"score": {"name":"@s","objective":"prepScore"},"color":"white"}]}
