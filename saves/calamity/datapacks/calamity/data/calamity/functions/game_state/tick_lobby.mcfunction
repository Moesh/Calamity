...
execute as @a[scores={startMatch=1..}] at @s run function calamity:player/trigger/start_match

# If a game start is happening, check to see if players want to cancel it
execute as @a[scores={cancelStart=1..}] at @s if score StartingMatch gameVariable matches 1 run function calamity:player/trigger/cancel_start
