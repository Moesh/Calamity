...
# We only really need to fix the wall at around the same y layer as the explosion happened. We will
#   just do a relative fill from a y layer a little under the explosion up to a y layer a little
#   higher. Now, there is a problem: /fill doesn't work if you try to fill blocks outside the world.
#   This means that if we try and fill down at layer 0 it will fail since we can't fill from
#   -8 up to +8 as -8 is outside the world. We also don't want to fill moving_piston blocks above
#   the arena height since the player can't get there anyways.
