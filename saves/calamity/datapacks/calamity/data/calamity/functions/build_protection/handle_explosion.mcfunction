...
# The "wall" moving_piston blocks are marked by marker blocks at layer y=254.
# The "roof" moving_piston blocks are marked by marker blocks at layer y=255. This is the in bounds region.
...
# First check to see if the y height is in range of the arena height. If it is, we have to search y=255 for the bounds region.
# Since this will be a search over an x/z plane, search y=254 at the same time.
# If the y height is not in range of the arena height, only search the x/z plane for y=254.
#
